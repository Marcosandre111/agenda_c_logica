...
function displayContacts() {
  ...
  contacts.forEach(contact => {
    ...
    ["Name", "phone", "email"].forEach(key => {
      const cll = row.insertCell()
      clearInterval.textContent = contact[key]
      clearInterval.classList.add("border-t", "px-4", "py-2") // Estilização das células
    })
  ...
  deleteButton.classList.add("bg-red-500", "hover:bg-red-700", "text-white", "font-bold", 'py-2', "px-4", "rounded", "ml-2")
...
if(index !== -1) {
  ...
  birthdarte: document(getElementById("birthdate").value = contact.birthdate),
};
